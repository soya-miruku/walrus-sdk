...
export function createWalrusClient(options: ClientOptions = {}) {
	...
	const maxUnknownLengthUploadSize =
		options.maxUnknownLengthUploadSize ||
			...
			try {
				...
				if (error instanceof Error) {
					...
					throw createWalrusError(
						`Failed to store JSON data: ${error.message}`,
						{ cause: error },
					);
				}
		...
		return logger.timeAsync("storeFromStream", async () => {
			...
			try {
				while (true) {
					...
					logger.debug("Read stream chunk", {
					...
					if (totalLength > maxUnknownLengthUploadSize) {
						...
						throw new Error(
							`Stream size exceeds maximum allowed (${maxUnknownLengthUploadSize} bytes). Use a file or buffer with known size instead.`,
						);
					}
				}
			} finally {
			...
			logger.debug("Combining stream chunks", {
	...
	const storeFromURL = (
		...
		return logger.timeAsync("storeFromURL", async () => {
			...
			if (!response.ok) {
				...
				throw new Error(
					`Failed to download from URL ${url}: HTTP status ${response.status}`,
				);
			}
	...
	const read = async (
		...
		return logger.timeAsync("read", async () => {
			logger.info("Reading blob", {
				blobId,
				hasDecryption: !!options.encryption,
			});
			...
			if (options.encryption) {
				...
				const decryptedData = await logger.timeAsync("decrypt", () =>
					cipher.decrypt(data),
				);
	...
	const readJSON = <T = unknown>(
		...
		return logger.timeAsync("readJSON", async () => {
			try {
				...
				logger.debug("JSON string decoded", {
				...
				logger.error("Failed to read JSON data with unknown error", {
					blobId,
				});
	 ...
	 */
	const readToStream = async (
		blobId: string,
		options: ReadOptions = {},
	): Promise<ReadableStream<Uint8Array>> => {
		return logger.timeAsync("readToStream", async () => {
			logger.info("Reading blob to stream", {
				blobId,
				hasDecryption: !!options.encryption,
			});
			...
			if (options.encryption) {
				...
				logger.debug("Creating stream from decrypted data", {
					size: data.byteLength,
				});
	 ...
	 */
	const head = (blobId: string): Promise<BlobMetadata> => {
	 ...
	 */
	const getAPISpec = (isAggregator: boolean): Promise<Uint8Array> => {
