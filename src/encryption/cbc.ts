...
export function createCBCCipher(options: CipherOptions): ContentCipher {
	...
	if (!options.iv) {
		throw createWalrusError(
			"Initialization vector (IV) is required for CBC mode",
		);
	}
	...
	const importKey = async (): Promise<CryptoKey> => {
		...
		try {
			...
			throw createWalrusError("Failed to import key for AES-CBC", {
				cause: error,
			});
		}
	};
	 ...
	 */
	const readStreamToUint8Array = async (
	...
	const encrypt = async (data: Uint8Array): Promise<Uint8Array> => {
		...
		try {
			...
			const ciphertext = new Uint8Array(
				await subtle.encrypt({ name: "AES-CBC", iv }, cryptoKey, padded),
			);

			// Combine magic bytes, IV, and ciphertext
			const result = new Uint8Array(
				MAGIC_BYTES.length + IV_LENGTH + ciphertext.length,
			);
	...
	const decrypt = async (data: Uint8Array): Promise<Uint8Array> => {
		...
		const extractedIv = data.slice(
			MAGIC_BYTES.length,
			MAGIC_BYTES.length + IV_LENGTH,
		);
