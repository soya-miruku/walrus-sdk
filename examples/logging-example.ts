...
import { LogLevel, configureLogger, createWalrusClient } from "../src";
...
async function main() {
	...
	console.log(
		"Demonstrating logger configuration and performance tracking in Walrus SDK",
	);
	...
	try {
		...
		const largeRetrieved = await client.readJSON<LargeData>(
			largeResponse.blob.blobId,
		);
		console.log(
			`Large data retrieved, contains ${largeRetrieved.items.length} items`,
		);
	} catch (error) {
