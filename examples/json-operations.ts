...
async function main() {
	...
	try {
		...
		console.log(
			`Will be available until epoch: ${storeResponse.blob.endEpoch}`,
		);
		...
		const typedData = await client.readJSON<ConfigData>(
			storeResponse.blob.blobId,
		);
		...
		console.log(
			`Generated encryption key: ${bytesToHex(key).substring(0, 16)}...`,
		);
		...
		console.log(
			`Encrypted JSON stored with blob ID: ${encryptedResponse.blob.blobId}`,
		);
